...
class InventoryService:
    ...
    def reserve_stock_for_order(self, order_items: List[Dict], order_id: str, 
                               expiration_minutes: int = 30) -> List[StockReservation]:
        """
        ...
        try:
            ...
            for item in sorted_items:
                ...
                if lock.acquire(blocking=True, timeout=30):
                    ...
                    raise ValidationError(f"Could not acquire lock for product {product_id}")
            ...
            for reservation in reservations:
                try:
                    ...
                    logger.error(f"Error releasing reservation during cleanup: {cleanup_error}")
            ...
            with transaction.atomic():
                ...
                for item in order.items.all():
                    ...
                    if success:
                        ...
                        product.stock_quantity = models.F('stock_quantity') - item.quantity
                ...
                try:
                    with transaction.atomic():
                        ...
                        self._log_stock_movement(
                            ...
                            reference_id=str(reservation.order_id) if reservation.order_id else None
                        )
                        
                        logger.info(f"Released expired reservation: {reservation}")
                        
                except Exception as e:
                    logger.error(f"Error releasing expired reservation {reservation.id}: {e}")
    ...
    def adjust_stock(self, product_id: int, new_quantity: int, reason: str = "Manual adjustment") -> bool:
        """
    ...
    def _log_stock_movement(self, product: Product, movement_type: str, 
                           quantity: int, reason: str, reference_id: str = None) -> StockMovement:
        """
