...
class AuditEvent(TimeStampedModel):
    ...
    user_email = models.EmailField(blank=True, help_text="Stored for historical reference")
    ...
    # Compliance fields
    regulation_tags = models.JSONField(default=list, blank=True, help_text="Applicable regulations (HIPAA, FDA, etc.)")
    retention_until = models.DateTimeField(null=True, blank=True, help_text="When this record can be purged")
    is_sensitive = models.BooleanField(default=False, help_text="Contains sensitive data")
    
    # System context
    process_id = models.CharField(max_length=50, blank=True, help_text="System process identifier")
    correlation_id = models.UUIDField(null=True, blank=True, db_index=True, help_text="Groups related events")
    ...
    def __str__(self):
        return f"{self.action_type} by {self.user_email or 'System'} at {self.created_at}"
...
class DataAccessLog(TimeStampedModel):
    ...
    # What was accessed
    resource_type = models.CharField(max_length=100, db_index=True)  # 'order', 'product', 'customer', etc.
    ...
    query_filters = models.JSONField(default=dict, blank=True, help_text="Applied filters/search terms")
...
class SecurityEvent(TimeStampedModel):
    ...
    event_category = models.CharField(max_length=30, choices=EVENT_CATEGORIES, db_index=True)
    severity = models.CharField(max_length=10, choices=SEVERITY_LEVELS, db_index=True)
    
    # Event details
    title = models.CharField(max_length=255)
    description = models.TextField()
    raw_data = models.JSONField(default=dict, help_text="Raw event data for investigation")
    ...
    response_actions = models.JSONField(default=list, help_text="Automated actions taken")
