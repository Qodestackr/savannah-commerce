...
class AuditEventAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'event_id_short', 'action_type', 'risk_level', 'user_email', 
        'target_object', 'created_at', 'is_sensitive', 'regulation_tags_display'
    ]
    list_filter = [
        'action_type', 'risk_level', 'is_sensitive', 'created_at',
        ('content_type', admin.RelatedOnlyFieldListFilter),
        ('user', admin.RelatedOnlyFieldListFilter),
    ]
    search_fields = [
        'user_email', 'description', 'object_repr', 'ip_address', 'request_path'
    ]
    readonly_fields = [
        'event_id', 'created_at', 'updated_at', 'correlation_id',
        'duration_since_event', 'additional_data_formatted'
    ]
    ...
    fieldsets = (
        ('Event Information', {
            'fields': (
                'event_id', 'action_type', 'risk_level', 'description',
                'created_at', 'duration_since_event'
            )
        }),
        ('User Context', {
            'fields': (
        ...
        ('Compliance', {
            'fields': (
                'regulation_tags', 'retention_until', 'is_sensitive'
            )
        }),
        ('System Context', {
            'fields': (
                'process_id', 'correlation_id', 'additional_data_formatted'
            ),
            'classes': ('collapse',)
        })
    )
    ...
    def regulation_tags_display(self, obj):
        ...
        for tag in obj.regulation_tags:
            ...
            badges.append(f'<span style="background-color: {color}; color: white; '
...
class DataAccessLogAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'user', 'access_type', 'resource_type', 'record_count',
        'created_at', 'is_suspicious', 'risk_score'
    ]
    list_filter = [
        'access_type', 'resource_type', 'is_suspicious', 'created_at',
        ('user', admin.RelatedOnlyFieldListFilter)
    ]
    search_fields = [
        'user__email', 'resource_type', 'api_endpoint', 'ip_address'
    ]
    ...
    fieldsets = (
        ...
        ('Request Context', {
            'fields': (
                'ip_address', 'user_agent', 'api_endpoint', 'query_filters_formatted'
            )
        }),
        ('Security Assessment', {
            'fields': (
...
class SecurityEventAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'title', 'event_category', 'severity', 'source_ip', 
        'user', 'created_at', 'is_resolved', 'resolved_by'
    ]
    list_filter = [
        'event_category', 'severity', 'is_resolved', 'created_at',
        ('user', admin.RelatedOnlyFieldListFilter),
        ('resolved_by', admin.RelatedOnlyFieldListFilter)
    ]
    search_fields = [
        'title', 'description', 'source_ip', 'user__email'
    ]
    readonly_fields = [
        'created_at', 'updated_at', 'raw_data_formatted', 'response_actions_formatted'
    ]
    date_hierarchy = 'created_at'
    ordering = ['-created_at']
    ...
    fieldsets = (
        ('Event Information', {
            'fields': (
                'title', 'event_category', 'severity', 'description', 'created_at'
            )
        }),
        ('Source Information', {
            'fields': (
                'source_ip', 'user_agent', 'user'
            )
        }),
        ('Resolution', {
            'fields': (
        ...
        ('Automated Response', {
            'fields': (
                'auto_response_taken', 'response_actions_formatted'
            ),
            'classes': ('collapse',)
        }),
        ('Raw Data', {
    ...
    def mark_resolved(self, request, queryset):
        ...
        self.message_user(
            request,
            f'Successfully resolved {count} security events.'
        )
    ...
    def mark_unresolved(self, request, queryset):
        ...
        self.message_user(
            request,
            f'Successfully marked {count} security events as unresolved.'
        )
...
class ComplianceReportAdmin(admin.ModelAdmin):
    ...
    list_display = [
        ...
        'generated_by', 'created_at', 'file_hash'
    ]
    list_filter = [
    ...
    readonly_fields = [
        ...
        'summary_formatted', 'findings_formatted', 'recommendations_formatted'
    ]
    date_hierarchy = 'created_at'
    ordering = ['-created_at']
    ...
    fieldsets = (
        ('Report Information', {
            'fields': (
                'title', 'report_type', 'generated_by', 'created_at'
            )
        }),
        ('Date Range', {
            'fields': (
    ...
    def has_permission(self, request):
        ...
        return (
            ...
            (request.user.is_superuser or 
             request.user.groups.filter(name='audit_staff').exists())
        )
