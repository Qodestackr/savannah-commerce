...
class AuditTrailMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        _thread_locals.session_key = request.session.session_key if hasattr(request, 'session') else ''
    ...
    def process_response(self, request, response):
        ...
        for attr in ['correlation_id', 'user', 'ip_address', 'user_agent', 
                     'request_method', 'request_path', 'session_key']:
...
def create_audit_event(action_type, description, **kwargs):
    ...
    audit_data = {
        ...
        'user_email': context['user'].email if context['user'] and context['user'].is_authenticated else '',
...
class AuditableMixin:
    ...
    def _calculate_risk_level(self, action_type, changed_fields):
        ...
        high_risk_fields = ['email', 'is_staff', 'is_superuser', 'price', 'stock_quantity']
